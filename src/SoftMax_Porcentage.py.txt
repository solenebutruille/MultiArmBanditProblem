...
def graphic_display(final_values_soft_max1, final_values_soft_max2, final_values_soft_max3, my_time):
    pyplot.plot(range(0, len(final_values_soft_max1[0])), np.mean(final_values_soft_max1, axis=0)*100, "b-", linewidth = 2, label = "temperature = 6")
    pyplot.plot(range(0, len(final_values_soft_max2[0])), np.mean(final_values_soft_max2, axis=0)*100, "r-", linewidth = 2, label = "temperature = 2")
    pyplot.plot(range(0, len(final_values_soft_max3[0])), np.mean(final_values_soft_max3, axis=0)*100, "g-", linewidth = 2, label = "temperature = 0")
    pyplot.legend(loc = "upper left")
    pyplot.xlabel("Steps")
    pyplot.ylabel("% Optimal Action")
    pyplot.ylim(0,100)
    pyplot.title("Learning with Soft_max")
    print("Time = ", time.time() - my_time)
    pyplot.show()

my_time = time.time()
final_values_soft_max1 = machine_learning(10, 2000, 1000, 6)
final_values_soft_max2 = machine_learning(10, 2000, 1000, 2)
